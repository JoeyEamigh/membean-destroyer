  ...
  constructor: function (d, e) {
    ...
    if (e && e.wrap) {
      ...
      this.el = this.el.wrap(
        typeof e.wrap == 'object' ? e.wrap : { cls: 'xresizable-wrap' }
      );
    ...
    for (var h = 0, k = n.length; h < k; h++) {
      if (n[h] && c[n[h]]) {
        var m = c[n[h]];
        this[m] = new Ext.Resizable.Handle(
          this,
          m,
    ...
    if (this.draggable) {
      ...
      this.dd.setHandleElId(
        this.resizeChild ? this.resizeChild.id : this.el.id
      );
      if (this.constrainTo) {
...
MB = (function () {
  ...
  return {
    ...
    AjaxGenericFailureMsg: function () {
      var a =
        "There seems to be some trouble reaching our server. Please check your network connection or wait for a minute and try again. If you are unable to resolve this issue please  <b> <a href='/contact'>contact us</b></a> and we will be happy to assist you!";
      Ext.Msg.show({
        msg: a,
        modal: false,
    ...
    AjaxGenericRetryMessage: function (a) {
      var b =
        "There's a problem connecting to Membean. Please wait while we retry.";
      Ext.Msg.show({
        msg: b,
        modal: true,
        icon: Ext.MessageBox.INFO,
    ...
    AjaxInternalErrorMessage: function () {
      var a =
        "We're sorry, but we've encountered an error that we couldn't automatically resolve. Please <b> <a href='/contact'>contact us</b></a> and we will be happy to assist you!";
      Ext.Msg.show({
        msg: a,
        modal: true,
        buttons: Ext.MessageBox.OK,
    ...
    AjaxNetworkErrorMessage: function () {
      var a =
        'Unable to reach Membean. Please check your internet connection or retry later.';
      Ext.Msg.show({
        msg: a,
        modal: true,
        buttons: Ext.MessageBox.OK,
    ...
    embedJwAudioPlayer: function (d) {
      ...
      var g = {
        ...
        modes: [
          { type: 'html5' },
          { type: 'flash', src: '/flash/player5.10.licensed.swf' },
          { type: 'download' },
        ],
      };
      jwplayer(b).setup(g);
    ...
    activateEmbeddedAudios: function () {
      var a = Ext.select('.embed-audio').each(function (d) {
        ...
        var b = { container: e, audio_name: c };
        if (c && e) {
    ...
    swfEmbedCallback: function (b) {
      if (!b.success) {
        ...
        if (a) {
          a.insertHtml(
            'afterBegin',
            "<h2 class='error'>Flash player needed </h2>"
          );
        }
      }
    },
    ...
    growl: function (b, c) {
      ...
      if ((el = Ext.get(c))) {
        var a = el.insertHtml(
          'afterBegin',
          "<div id='user-growl'>" + b + '</div>',
          true
        );
        a.slideIn('t', { useDisplay: true })
          .pause(1.5)
          .slideOut('t', { useDisplay: true });
      }
    },
    displayCalloutMsg: function (b, d, c, a) {
      if (b) {
        ...
        b.insertHtml(
          'afterBegin',
          "<div id='callout-box'><div class='content'><h3>" +
            d +
            '</h3><p>' +
            c +
            '</p><div></div>'
    ...
    sendRemoteMessage: function (a) {
      Ext.Ajax.request({ url: '/debugger/log', params: { text: a } });
    },
    closePanel: function () {
      Ext.getBody().on(
        'click',
        function (c, b) {
          ...
          if (a) {
            a.switchOff({ useDisplay: true, duration: 1 });
          }
        },
        this,
  ...
  a.idleTimer = function (g, f, e) {
    ...
    var d = a(f),
      ...
      b = function (m) {
        ...
        var k = a.Event(
          a.data(f, 'idleTimer', l.idle ? 'idle' : 'active') + '.idleTimer'
        );
        a(f).trigger(k);
      },
      c = function (j) {
...
Ext.extend(MB.Tester, Ext.util.Observable, {
  ...
  next_tester: function () {
    var a =
      this.current_level <= this.max_tested_levels
        ? this.level_testers[++this.current_level]
        : null;
    return a;
  },
  ...
  submit_test: function () {
    ...
    Ext.Ajax.request({
      ...
      params: { data: this.server_string() },
      method: 'POST',
  ...
  update_word: function (a) {
    this.el
      .update(a)
      .fadeIn({ easing: 'backIn', callback: this.progress, scope: this });
  },
  progress: function (b) {
    this.level_progress += 1;
...
Ext.extend(MB.LevelTester, Ext.util.Observable, {
  ...
  response: function (a) {
    if (a === 'yes') {
      ...
      this.tested_words.push({ word: this.current_word, response: 'no' });
    }
  },
  pass: function () {
...
Ext.onReady(function () {
  ...
  Ext.select('#faqs .faq-control').on(
    'click',
    function (h, d) {
      h.stopEvent();
      var g = Ext.get(d);
...
MB.LandingSlideShow = Ext.extend(Ext.util.Observable, {
  constructor: function (c, b, d, a) {
    ...
    this.nextAdvanceTask = new Ext.util.DelayedTask(
      this.next.createDelegate(this)
    );
...
Ext.onReady(function () {
  if (Ext.getBody().hasClass('landing')) {
    ...
    if (Ext.get('slideshow-nav')) {
      var a = new MB.LandingSlideShow(
        Ext.select('.slide'),
        Ext.get('slideshow-left'),
        Ext.get('slideshow-right'),
        Ext.get('slideshow-nav')
      );
    }
  }
});
...
MB.SlideShow = Ext.extend(Ext.util.Observable, {
  ...
  initMarkup: function () {
    var a = Ext.DomHelper;
    this.size = 0;
    this.els.container = a.append(
      this.el,
      { cls: 'mb-slideshow-container' },
      true
    );
    this.els.slidesWrap = a.append(
      this.els.container,
...
Ext.onReady(function () {
  ...
  Ext.select('textarea').addClassOnFocus('active');
...
MB.affliateCharts = function () {
  ...
  var f = new google.visualization.PieChart(
    document.getElementById('all-signups-chart')
  );
  chart_options = k;
  chart_options.title = 'Introduced By Product';
  f.draw(g, chart_options);
  var b = new google.visualization.PieChart(
    document.getElementById('paid-signups-chart')
  );
  chart1_options = k;
  ...
  var h = Ext.util.JSON.decode(
    Ext.get('all-signups-by-month-data').dom.innerHTML
  );
  j.addRows(h);
  var d = new google.visualization.AreaChart(
    document.getElementById('all-signups-by-month')
  );
  d.draw(j, {
    width: 400,
    height: 240,
    chartArea: { left: 50, top: 50, width: '95%' },
    hAxis: {
      slantedText: true,
      textPosition: 'out',
      textStyle: { fontSize: 9, color: '#3366cc' },
    },
    legend: 'top',
  ...
  if (b) {
    var a = new Ext.ux.grid.TableGrid(b, {
      ...
      columns: [
        ...
        { sortable: true },
        { sortable: true },
        { sortable: true, renderer: Ext.util.Format.dateRenderer('M d, y') },
      ],
    });
    a.render();
  }
...
MB.AssessmentTimer = Ext.extend(MB.Timer, {
  ...
  start: function () {
    ...
    if (this.nsecs > this.dangerTimeout) {
      setTimeout(
        this.danger.createDelegate(this),
        (this.nsecs - this.dangerTimeout) * 1000
      );
    }
    if (this.nsecs > this.warningTimeout) {
      setTimeout(
        this.warning.createDelegate(this),
        (this.nsecs - this.warningTimeout) * 1000
      );
    }
  },
  danger: function () {
  ...
  addListeners: function () {
    ...
    this.form.on(
      ...
      function (f, c) {
        ...
        if (d.length > 0) {
          var b = confirm(
            "Some questions are unanswered. Press 'Cancel' if you want to answer these questions before submitting the test. Press 'OK' if you want to submit the test."
          );
          for (var a = 0; a < d.length; ++a) {
            el = d[a];
  ...
  preventFocusChange: function () {
    ...
    var c = function () {
      ...
      if (e && a.delayExceeded) {
        ...
        a.abortOnFocusLoss();
        alert(
          'You moved away from the assessment window again. Terminating test!'
        );
      } else {
        if (!e) {
          var d = alert(
            'Do not leave this window. Your test will be terminated if you do this again. Click OK right now!'
          );
...
MB.activateEducatorBookSearch = function () {
  var b = new Ext.data.Store({
    proxy: new Ext.data.HttpProxy({ url: '/booklist.json', method: 'GET' }),
    reader: new Ext.data.JsonReader(
      { root: 'rows', totalProperty: 'results', id: 'book_id' },
      [
...
MB.activateEducatorWordSearch = function () {
  var b = new Ext.data.Store({
    ...
    reader: new Ext.data.JsonReader(
      { root: 'rows', totalProperty: 'results', id: 'word_id' },
      [
        { name: 'wordform', mapping: 'wordform' },
        { name: 'definition', mapping: 'defn' },
      ]
    ),
  });
  var c = new Ext.XTemplate(
...
MB.activateEducatorRootSearch = function () {
  var b = new Ext.data.Store({
    proxy: new Ext.data.HttpProxy({ url: '/treelist.json', method: 'GET' }),
    reader: new Ext.data.JsonReader(
      { root: 'rows', totalProperty: 'results', id: 'tree_id' },
      [
        { name: 'rootform', mapping: 'rootform' },
        { name: 'label', mapping: 'label' },
        { name: 'meaning', mapping: 'defn' },
      ]
    ),
  });
  var c = new Ext.XTemplate(
...
MB.trainingSessionChart = function () {
  if (Ext.get('ts-chart')) {
    ...
    j.addRows(c);
    var g = new google.visualization.AnnotatedTimeLine(
...
MB.nonFlashTrainingSessionChart = function () {
  if (Ext.get('ts-chart-noflash')) {
    ...
    var c = Ext.util.JSON.decode(Ext.get('ts-chart-data').dom.innerHTML)[
      'data'
    ];
    a.addRows(c);
    var b = new google.visualization.AreaChart(
      document.getElementById('ts-chart-noflash')
    );
    b.draw(a, {
      width: 600,
      height: 240,
      chartArea: { left: 30, top: 20, width: '90%' },
      hAxis: {
        slantedText: true,
        format: 'MMM, d',
        textPosition: 'out',
      ...
      legend: 'top',
      title:
        'Study time over the last 60 days (you want this to be evenly spaced)',
    });
  }
};
MB.trainingSummaryByDateRange = function () {
  if (Ext.get('study-by-date')) {
    Ext.select('#study-by-date form').on('submit', function (d) {
      ...
      var c = Ext.select('#study-by-date form')
        .first()
      ...
      if (a && f && b) {
        Ext.Ajax.request({
          ...
          success: function (k) {
            ...
            Ext.get('study-by-date-result').dom.innerHTML =
              "You've trained <em> " +
              l +
              '</em> min. in the selected time range.';
            var g = e.data.name;
            var n = e.data.performance['new'];
            var j = e.data.performance.incorrect;
            var m = e.data.performance.correct;
            Ext.get('report-chart-data').dom.innerHTML =
              '[["' + g + '",' + [n, j, m] + ']]';
            var o = j + m;
            var h =
              "You've seen <em>" +
              n +
              '</em> new word(s) and answered <em>' +
              o +
              '</em> question(s). ';
            if (o > 0) {
              h +=
                'Of these you’ve answered <em>' +
                j +
                '</em> incorrectly and <em>' +
                m +
                '</em> correctly.';
            }
...
MB.WordLayout = (function () {
  var a = [
    { name: 'Context & Definition', id: 'l-context', pos: 0, col: 0 },
    { name: 'Memory Hooks', id: 'l-memhook', pos: 1, col: 0 },
    { name: 'Example Sentences', id: 'l-examples', pos: 2, col: 0 },
    { name: 'Word Ingredients', id: 'l-structure', pos: 0, col: 1 },
    { name: 'Word Theater', id: 'l-theater', pos: 1, col: 1 },
    { name: 'Word Constellation', id: 'l-wordmap', pos: 2, col: 1 },
    { name: 'Related Words', id: 'l-relatedwords', pos: 3, col: 1 },
    { name: 'Word Variants', id: 'l-variants', pos: 4, col: 1 },
  ];
  var c = function (e) {
    var d = [];
    var f = a
    ...
    f.forEach(function (g) {
      d.push({ collapsible: false, title: g.name, frame: false, id: g.id });
    });
    return d;
  };
  var b = function () {
        ...
        items: [
          {
            ...
            border: false,
            items: [
              { columnWidth: 0.55, id: 'wcol0', items: c(0) },
              { columnWidth: 0.45, id: 'wcol1', items: c(1) },
            ],
            listeners: {
  ...
  if (b) {
    var a = new Ext.ux.grid.TableGrid(b, {
      ...
      columns: [
        ...
        { sortable: true, renderer: Ext.util.Format.dateRenderer('M d, y') },
        { sortable: true },
        { sortable: true, renderer: Ext.util.Format.dateRenderer('M d') },
        { sortable: true },
        { sortable: true },
      ],
    });
...
MB.showClassReadiness = function () {
  ...
  if (h && e) {
    ...
    h.on('change', function (m) {
      ...
      readyStudentsEl = Ext.select(
        '#assessment-readiness .students-ready em'
      ).first();
      readyStudentsEl.dom.innerHTML = j.join(', ');
      readyStudentsEl.highlight();
      notReadyStudentsEl = Ext.select(
        '#assessment-readiness .students-not-ready em'
      ).first();
...
MB.activityChart = function () {
  if (Ext.get('level-chart')) {
    ...
    var k = {
      ...
      chartArea: { left: 30, top: 10, bottom: 10 },
      pieSliceText: 'value',
      pieSliceTextStyle: { color: 'black' },
      colors: ['#CDC4F1', '#BAEDED', '#9EB2DD', '#E3EBFE', '#AD9FDE'],
      height: 150,
    };
    var e = new google.visualization.PieChart(
      document.getElementById('level-chart')
    );
    e.draw(h, k);
  }
  ...
  if (Ext.get('report-chart')) {
    ...
    b.addRows(f);
    var c = new google.visualization.BarChart(
      document.getElementById('report-chart')
    );
    c.draw(b, {
      width: 490,
      height: f.length * 40,
      chartArea: { top: 20, left: 75, width: '98%', bottom: 0, height: '90%' },
      legend: 'bottom',
      isStacked: 'true',
      title: 'Word study for this period',
      fontName: 'Georgia',
      colors: ['#C2B93E', '#FFAA00', '#808F5D'],
      hAxis: { textPosition: 'in', gridlineColor: '#DDDDDD' },
...
MB.teacherResetPassword = function () {
  Ext.getBody().on(
    ...
    function (g, d) {
      ...
      if (c) {
        a +=
          ' Students who have school provided emails can reset their own passwords.';
      }
...
Ext.onReady(function () {
  ...
  var d = m
    ...
    .map(x)
    .on('click', function (w, h) {
      l(w);
    });
  m.append('text')
    .attr('transform', 'translate(-6,' + f * 3.5 + ')rotate(-90)')
  ...
  var c = d3
    ...
    .attr('transform', 'translate(' + f * 54 + ',' + f + ')');
  c.selectAll('text.weekday-name')
  ...
  function l(z) {
    ...
    function K(R) {
      var S = d3
        .select(I)
        .selectAll('div.session-activity')
        .data(R, function (T) {
          return T.start;
        });
      S.enter()
        .append('div')
        .attr('class', 'session-activity')
        .append('h3')
        .text(function (T) {
          return T.start;
        });
...
window.Constellation = (function (a) {
  ...
  c.d = function (e, f, d) {
    if (!c.o(e, f)) {
      Object.defineProperty(e, f, { enumerable: true, get: d });
    }
  };
  c.r = function (d) {
  ...
  function (aH, al, az) {
    ...
    function D(aJ, aI) {
      ...
      aJ.prototype = Object.create(aI && aI.prototype, {
        constructor: { value: aJ, writable: true, configurable: true },
      });
      if (aI) {
        S(aJ, aI);
      }
      ...
      T(aP, [
        ...
        },
        {
          ...
          value: function aR() {
            ...
            aY = Math.max.apply(
              Math,
              this.childNodes.map(function (aZ) {
                return aZ.maxBloom();
              })
            );
          ...
          if (Object.getOwnPropertyDescriptors) {
            ...
            M(Object(aJ)).forEach(function (aL) {
              Object.defineProperty(
                aK,
                aL,
                Object.getOwnPropertyDescriptor(aJ, aL)
              );
            });
          }
        }
      ...
      C(aJ, [
          ...
          value: function aM(aU, aT) {
            ...
            if (aU) {
              this.toolTipRectangle = aq(
                { x: this.x - aR / 2, y: this.y + aP / 2 },
                aS
              );
            } else {
              this.toolTipRectangle = aq(
                { x: this.x - aR / 2, y: this.y - 105 },
                aS
              );
            }
            var aO = new createjs.Graphics()
              .beginFill(aQ)
              .drawRoundRect(
        ...
        },
        {
          ...
          value: function aK(aS, aQ, aU) {
            ...
            var aP = new createjs.Graphics()
              .beginFill(aR)
              .drawPolyStar(aV.x, aV.y, 12, 3, 0, aT);
          ...
          value: function aL(aU, aT) {
            ...
            if (aU) {
              aV = aq(
                aq({}, aS),
                {},
                { x: this.x - aQ / 2 + aO, y: this.y + aW / 2 + aR }
              );
            } else {
              aV = aq(
                aq({}, aS),
                {},
                { x: this.x - aQ / 2 + aO, y: this.y - 100 + aR }
              );
            }
            aP.set(aV);
            return aP;
    ...
    var ak = (function (aL) {
      ...
      ao(aN, [
        ...
        },
        {
          key: 'setupAnimation',
          value: function aK() {
            return (this.tween = createjs.Tween.get(this, { paused: true }).to(
              {
    ...
    var H = {
      0: { code: 'hypernym', description: 'kind of', direction: 'up' },
      1: { code: 'antonym', description: 'opposite of', direction: 'none' },
      2: {
        code: 'entailment',
        description: 'if true then also true',
        direction: 'up',
      },
      3: { code: 'cause', description: 'causes', direction: 'none' },
      4: { code: 'similar_to', description: 'similar to', direction: 'none' },
      5: {
        code: 'pertainym',
        description: 'of or pertaining to',
        direction: 'up',
      },
      6: { code: 'attribute', description: 'attribute of', direction: 'down' },
      7: { code: 'see_also', description: 'see also', direction: 'none' },
      ...
      E(aP, [
        ...
        },
        {
          ...
          value: function aR() {
            this.centerPoint = new createjs.Point(
              this.wordmap.stage.width / 2,
              this.wordmap.stage.height / 2
            );
            this.rootNode = new aj(this.wf, this, this.mode);
            this.rootNode.updatePosition(
              this.centerPoint.x,
              this.centerPoint.y
            );
          },
        },
        {
          ...
          value: (function (aW) {
            ...
            var aV = this;
            return fetch(
              'https://cdn1.membean.com/public/data/wnjson2/'.concat(
                this.wf,
                '.json'
              )
            )
        ...
        },
        {
          ...
          value: function aM(aW, a0) {
            ...
            for (aY = 0, a3 = aW.length; aY < a3; aY++) {
              ...
              a7 =
                a1.l === 1
                  ? new c(a0, aX, true, this.wordmap.stage)
                  : new c(a0, aX, false, this.wordmap.stage);
              this.nodes.push(aX);
              this.links.push(a7);
              if (a1.children) {
                ...
                function () {
                  ...
                  for (a1 = a9 = 0, bb = aW.length; a9 < bb; a1 = ++a9) {
                    ...
                    while (a8) {
                      ...
                      a7 = this._coordinatesFromAngle(
                        a6.distance_from_center() + a6.jitter,
                        a6.theta()
                      );
      ...
      B(aJ, [
        ...
        },
        {
          ...
          value: function aM() {
            ...
            aQ.innerHTML =
              '<button class="wordmap-action back" data-action="back" disabled>&#60;</button><button class="wordmap-action forward" data-action="forward" disabled>&#62;</button>';
    ...
    if (Ext.get('ikt')) {
      Ext.get('ikt').on('click', function (b, a) {
        ...
        Ext.Ajax.request({
          ...
          failure: MB.AjaxGenericFailureMsg,
          method: 'POST',
          params: { _method: 'PUT', skip_this: true },
          success: function (c) {
...
MB.activateFastDict = function () {
  this.fastdict = new MB.FastDict({ wrapperEl: 'fastdicts' });
    ...
    function (a) {
      ...
      if (Shadowbox) {
        Shadowbox.open({
          ...
          options: {
            onFinish: function (b) {
              new MB.FastDict({ wrapperEl: Ext.get('sb-player') });
            },
          },
        });
      }
...
var addHookT = new Ext.Template(
  '<div class="hook">',
  '<span url="{new_url}"></span>',
  '<p><img src="{img}"/>{text}</p></div>'
);
var addHookT_noimg = new Ext.Template(
  '<div class="hook">',
  '<span url="{new_url}"></span><p>{text}</p>'
);
var updateHookT = new Ext.Template(
  '<span url="{new_url}"></span>',
  '<p><img src="{img}"/>{text}</p>'
);
var updateHookT_nomimg = new Ext.Template(
  '<span url="{new_url}"></span>',
  '<p>{text}</p>'
);
...
MB.showMHPanel = function (c) {
  ...
  if (MB.MHCarousel) {
    ...
    var b = c.insertHtml(
      'afterBegin',
      "<div id='memhook-slideshow' style='display:none'/>",
      true
    );
    ...
    function () {
      ...
      a.each(function (l) {
        ...
        var k = Ext.DomHelper.append(document.body, {
          ...
          cls: 'memhook-item',
          children: [
            { tag: 'p', html: h + l.data.hook },
            { tag: 'a', cls: 'useme', html: 'Use This' },
          ],
        });
        MB.MHCarousel.add(k);
...
MB.Question = Ext.extend(Ext.util.Observable, {
  ...
  warnSpelling: function (e, d, a) {
    ...
    var b = this.config.resultEl || Ext.get('answer-status');
    b.update(
      "<div class='spelling-error'><span class='incorrect'>" +
        e +
        "</span><span>&rarr;</span><span class='success'>" +
        d +
        '</span><h2>Check Spelling!<br/><span>off by ' +
        a +
        ' ' +
        c +
        '</span></h2></div>'
...
MB.ClozeQuestion = Ext.extend(MB.Question, {
  ...
  showCorrectAnswer: function () {
    this.formInput.dom.value = this.answer.substr(
      this.firstLetterCorrection,
      this.answer.length - this.firstLetterCorrection
    );
    this.formInput.removeAllListeners();
  },
...
MB.Review.activateClozeQuestion = function () {
  ...
  var a = new MB.ClozeQuestion({
    timer: { renderTo: 'timer-container', timeout: b },
    callback: {
      correct: function () {
        a.updateResult(true);
      },
...
MB.Review.activateMCQuestion = function () {
  ...
  var b = new MB.MCQuestion(Ext.select('#choice-section .choice'), {
    timer: { renderTo: 'timer-container', timeout: a },
    callback: {
      correct: function () {
        b.updateResult(true);
      },
...
MB.Sticky = (function () {
  return {
    ...
    save: function (b) {
      ...
      var a =
        b.getAttribute('data-update-url') || b.getAttribute('data-create-url');
      Ext.Ajax.request({
        url: a,
...
MB.TrainerRenderer = {
  render: function (d, b, c, e) {
    ...
    if (a) {
      ...
      d.update(b.responseText, c.loadScripts, e)
        .slideIn('r', {
          duration: 0.75,
          concurrent: true,
          easing: 'easeOutStrong',
        })
        .fadeIn({ duration: 0.75, concurrent: true, easing: 'easeOutStrong' });
    }
  },
};
...
MB.Trainer = Ext.extend(Ext.util.Observable, {
  ...
  setupUpdater: function () {
    ...
    this.contentUpdater.indicatorText =
      "<div class='trainer-loading'><a href='" +
      window.location +
      "'>Taking too long? click me</a></div>";
    ...
    this.contentUpdater.on(
      'failure',
      this.pageLoadFailure.createDelegate(this)
    );
  },
  setupHandlers: function () {
    Ext.get('training_sessions').on(
      'click',
      function (f, b) {
  ...
  advance: function (e, d) {
    ...
    this.activeTarget.insertHtml(
      'beforeEnd',
      "<input id='annotate_it' name='it' type='hidden' value=" +
        this.idlingData +
        '></input>'
    );
    this.activeTarget.insertHtml(
      'beforeEnd',
      "<input id='more_ts' name='more_ts' type='hidden' value=" +
        this.tsEncryptSeed(d) +
        '></input>'
    );
  ...
  pageLoadFailure: function (b, a) {
    var d =
      "If you are unable to resolve this issue please <b> <a href='/contact'>contact us</b></a> and we'll be happy to assist you!";
    var c = null;
    if (a.status == '500') {
      c =
        "Something went wrong. We've been informed of this. Please try again after some time. " +
        d;
      ...
      if (this.retries < this.config.maxRetries) {
        if (this.retries >= 1) {
          c =
            "You are currently experiencing problems with your network. Let's try again.";
          this.waitBox =
            this.waitBox ||
            Ext.Msg.wait(c, 'Trying again', {
              animate: true,
        ...
        this.waitBox.hide();
        c =
          "There's a problem connecting to Membean. This is usually because of internet connection problems with your machine or in rare cases because of downtime at our servers. Please try again in a little while. " +
          d;
        this.showFailureMsg(c);
      }
    }
  ...
  questionPageLoaded: function () {
    ...
    if (this.isQuestionAlreadyAnswered()) {
      ...
      if (a) {
        MB.displayCalloutMsg(
          a,
          'You just answered this, did you not?',
          '<b>Move on, let us</b>. Why spend time answering the same question, hmmm? Many questions to answer, we have. Yes, hmmm.',
          'yoda-tiny.jpg'
        );
      }
  ...
  advanceAfterAnsweringQuestion: function (a) {
    a.on(
      ...
      function (b) {
        ...
        this.advance.defer(c * 1000, this, [
          this.activeTarget,
          this.autoClickDetected,
        ]);
      },
      this
    );
  ...
  skip: function (b) {
    if (Ext.fly('skip-confirm')) {
      var a =
        "When a new word is first shown, you can mark it as 'known'. Such words are removed from further practice and added to the list of words that are ready for test day. Use this with caution and only if you can use the word comfortably in a sentence. <b>Do you know this word well?</b>";
      Ext.Msg.show({
        title: 'I Know This',
  ...
  warnOnContextUnanswered: function (c) {
    if (this.newWord() && !Ext.fly('choice-section').child('.correct')) {
      message =
        "Active learning improves recall. It's a good idea to answer the <b> Context Memlet quiz</b> when you first encounter a word. If you already know this word well, check the <b> I Know This (IKT)</b> box next to the word.";
...
MB.findOrCreateWordAudioPlayer = function () {
  ...
  if (a) {
    if (!Ext.get('word-audio-player')) {
      ...
      if (!window.wap) {
        window.wap = new MB.WordAudioPlayer({ container: 'word-audio-player' });
      }
    }
  }
  return window.wap;
    ...
    Ext.select('#tooltips-container .tooltip-marker').each(function (c) {
      ...
      if (f) {
        ...
        var d = new Ext.ToolTip({
          ...
          unstyled: true,
          showDelay: 650,
          bodyCfg: { cls: 'tbox', html: c.getAttribute('data-msg') },
          mouseOffset: [b, -10],
        });
      }
    });
      ...
      Ext.Ajax.request({
        ...
        success: function (c, d) {
          ...
          if (f.data.success == true) {
            ...
            e.addClass('success');
            e.dom.innerHTML =
              'Approved by <b>' + a.getAttribute('approver') + '</b>';
          }
        },
      });
    });
  ...
  a.on('mouseup', function () {
    ...
    if (c != '') {
      ...
      if (!d) {
        a.insertHtml(
          'beforeEnd',
          "<a id='save-cc-cues' class='x-short-btn-blue' href='#'>Save Cues</a>"
        );
      }
    }
  });
...
MB.zoomWordMap = function (c) {
  ...
  var b = {
    ...
    player: 'html',
    content:
      "<div id='wordmap-modal'><div id='wordmap-zoom-container'></div></div>",
    options: {
      onFinish: function () {
...
Ext.extend(MB.RootTree, Ext.util.Observable, {
  ...
  buildLayer: function () {
    ...
    if (!a) {
      a = this.parent
        .insertHtml('beforeEnd', "<div class='layer'/>", true)
  ...
  addTreeImage: function () {
    ...
    var b = d + '/' + c;
    var a = this.currlayer
      .insertHtml('beforeEnd', '<img src="' + b + '"/>', true)
      .setWidth(this.width)
  ...
  addTitle: function () {
    this.currlayer
      .insertHtml('afterBegin', '<h2>' + this.drootform + '</h2>', true)
      .addClass('title');
  },
  addSignboard: function () {
    var a = this.currlayer
      .insertHtml('beforeEnd', '<span>' + this.meaning + '</span>', true)
      .addClass('signboard');
  ...
  addGnarlyRoot: function () {
    var d = this.rootcoords[0],
      a = d[0],
      b = d[1];
    var c = this.currlayer
      .insertHtml('beforeEnd', '<span/>', true)
      .addClass('box-label gnarly-root');
    this.setLT(c, a, b).fadeIn();
    c.dom.innerHTML =
      '<strong>' + this.drootform + '</strong>: ' + this.meaning + '</span>';
  },
  addLeafs: function () {
    this.leafboxes = [];
    ...
    for (var d = 0; d < h; ++d) {
      var b = this.leafcoords[d],
        c = b[0],
        j = b[1];
      var f = this.currlayer
        .insertHtml('beforeEnd', "<span data-idx='" + d + "'/>", true)
        .addClass('box-label leaf-label');
      this.setLT(f, c, j);
  ...
  addLegend: function () {
    var a = this.currlayer.insertHtml(
      'beforeEnd',
      "<ul class='legend'><li class='membean'> Membean word</li><li class='non-membean'> Other word</li></ul>",
      true
    );
    this.setLT(a, 10, 400);
  },
  addRelated: function () {
    ...
    if (b > 0) {
      ...
      d += '</table></div>';
      this.relatedPanel = this.parent
        .insertHtml('beforeEnd', d, true)
        .setRight(0)
        .setBottom(0);
      ...
      function (d) {
        ...
        if (b) {
          ...
          this.currlayer
            .insertHtml(
              'afterBegin',
              '<span>' + a + "<a href='#', class='close'/></span>",
              true
            )
            .addClass('definition')
            .setXY([b.getLeft(), b.getTop()]);
        }
      },
      this,
      { delegate: '.leaf-label' }
...
Ext.onReady(function () {
  ...
  Ext.select('#wrotd-structure .wi-btn').on('click', function (d) {
    ...
    tree =
      t.dom.getAttribute('data-tree-id') ||
      t.parent().dom.getAttribute('data-tree-id');
    c.changeToTree(tree);
  });
  Ext.select('#root-collage').on(
    'mouseover',
    function (d) {
      d.stopEvent();
      t = Ext.get(d.getTarget());
